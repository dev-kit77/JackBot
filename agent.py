...
class Agent:
    ...
    def update(self, obs, action, reward):
        ...
        self.q_values[obs][action] = (
            self.q_values[obs][action] + self.learning_rate * error
        )
        ...
        for player in range(21, 3, -1):
            ...
            for dealer in range(2, 12):
                ...
                print(
                    " %s "
                    % (
                        ACTIONS[int(np.argmax(e))]
                        if not np.array_equal(e, np.zeros(2))
                        else "X"
                    ),
                    end=" ",
                )
        ...
        if not (episode % checkpoint) and episode > 0:
            print(
                ...
                % (
                    ...
                    checkpoint,
                    wins / checkpoint,
                    agent.new_states - nstates,
                    agent.total_states - tstates,
                    ((agent.new_states - nstates) / (agent.total_states - tstates)),
                )
            )
...
def test():
    ...
    print(
        "TEST: %i / %i = %f; %i / %i = %f"
        % (wins, N_TESTS, wins / N_TESTS, losses, N_TESTS, losses / N_TESTS)
    )
